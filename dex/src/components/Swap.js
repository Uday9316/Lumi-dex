...
// Define a function to fetch token balance using ethers.js
...
function Swap(props) {
  ...
  // Function to open the modal to select tokens
  ...
  // Function to modify selected token
  ...
  // Function to fetch and perform the swap
  async function fetchDexSwap() {
    // Validate token amount
    if (!tokenOneAmount || Number(tokenOneAmount) <= 0) {
      messageApi.open({
        type: 'error',
    ...
    try {
      // Fetch token balance
      const balance = await fetchTokenBalance(tokenOne.address, address);
      ...
      messageApi.open({
        type: 'error',
        content: 'Error performing swap: ' + error.message,
        duration: 1.5,
      });
